...
import {
  ...
  ClipboardList, ScanSearch, ShieldCheck
} from "lucide-react";
...
export const Route = createFileRoute("/services/")({
  head: () => ({
    meta: [
      { title: "Quality Assurance, Inspection & Surveying Services | Qualify Kenya" },
...
const services: Service[] = [
  {
    ...
    desc: "Independent marine inspection and survey support across Kenya and the region.",
  ...
  },
  {
    ...
    desc: "Cargo condition, quantity, loading and discharge verification.",
  ...
  },
  {
    ...
    desc: "Vessel condition, equipment status and readiness assessments.",
  ...
  },
  {
    ...
    desc: "Evidence-based cargo, vessel and equipment loss assessments.",
  ...
  },
  {
    ...
    pageLink: "/services/condition-surveys",
    desc: "Independent condition assessments for vessels, hulls, hatch covers, machinery, cargo and equipment.",
    context: "QUALIFY condition surveys provide a factual technical record before transactions, charter commitments, underwriting decisions or claims. Our surveyors inspect accessible systems and assets, grade findings and document evidence in a clear report.",
    how: [
      "Confirm survey scope, location and priority areas",
      "Attend vessel, cargo or equipment site",
      "Inspect accessible compartments, machinery, equipment and documentation",
      "Record deficiencies, condition grades and photographic evidence",
      "Issue condition report with practical recommendations",
    ],
    when: [
      "Before vessel purchase or charter",
      "For insurance underwriting or financing",
      "Before cargo handover or acceptance",
      "During periodic fleet or asset condition monitoring",
    ],
    who: "Vessel buyers, charterers, financiers, insurers, P&I clubs, fleet managers, legal teams.",
    deliv: ["Condition survey report", "Deficiency register", "Photographic evidence annex", "Certificate review notes", "Recommended actions"],
  },
  {
    icon: Award,
    title: "Quality Assurance & Compliance",
    pageLink: "/services/quality-assurance",
    desc: "Quality systems, controls and compliance readiness support.",
  ...
  },
  {
    ...
    pageLink: "/services/accreditation-support",
    desc: "KENAS readiness support for inspection bodies, medical laboratories, and testing or calibration laboratories.",
    context: "We help organisations prepare for accreditation by strengthening systems, technical records, competence evidence and impartiality controls. The work is practical and evidence-led, with a focus on closing gaps before formal assessment.",
    how: [
      "Accreditation readiness gap assessment",
      "Document and record review against the target standard",
      "Competence, impartiality and traceability evidence preparation",
      "Internal audit and corrective action support",
      "Final readiness review before assessment",
    ],
    when: [
      "Preparing for KENAS accreditation",
      "Responding to accreditation findings",
      "Expanding accreditation scope",
      "Strengthening laboratory or inspection-body systems",
    ],
    who: "Inspection bodies, testing laboratories, calibration laboratories, medical laboratories, technical managers.",
    deliv: ["Gap assessment report", "Accreditation readiness roadmap", "Documented evidence register", "Internal audit report", "Corrective action tracker"],
  },
  {
    icon: BadgeCheck,
    title: "Certification Support",
    pageLink: "/services/certification-support",
    desc: "Certification readiness, audits and corrective action support.",
  ...
  },
  {
    ...
    desc: "Practical training for inspection, quality and reporting teams.",
  ...
  },
  {
    icon: ClipboardList,
    title: "Auditing",
    pageLink: "/services/auditing",
    desc: "Internal audits, supplier audits, process audits, gap assessments and corrective action follow-up.",
    context: "QUALIFY audits are structured, impartial and improvement-oriented. We review evidence, interview process owners, observe operations and translate findings into clear corrective actions.",
    how: [
      "Agree audit scope, criteria and schedule",
      "Prepare audit plan and sampling approach",
      "Conduct document review, interviews and site observations",
      "Grade findings and identify root causes",
      "Issue audit report and corrective action tracker",
    ],
    when: [
      "Before certification or accreditation assessment",
      "When supplier performance needs verification",
      "After recurring process failures",
      "For periodic internal audit programmes",
    ],
    who: "QA managers, compliance teams, procurement teams, operations managers, executives.",
    deliv: ["Audit plan", "Audit report", "Findings register", "Corrective action tracker", "Follow-up verification notes"],
  },
  {
    icon: FileText,
    title: "Technical Reporting & Documentation",
    pageLink: "/services/technical-reporting",
    desc: "Clear technical reports for audits, claims and management review.",
  ...
  {
    icon: Cpu,
    title: "Quality Management Portal",
    pageLink: "/services/quality-system-implementation",
    desc: "Cloud QMS for documents, audits, CAPA and training records.",
  ...
  },
  {
    ...
    pageLink: "/services/sample-logistics",
    desc: "Laboratory identification, sample collection coordination, chain-of-custody and secure transport.",
    context: "We coordinate sample handling from collection through laboratory submission, preserving sample condition and maintaining a clear chain-of-custody record for technical, commercial or legal use.",
    how: [
      "Identify suitable accredited or reputable laboratory",
      "Coordinate sample collection and labelling",
      "Prepare chain-of-custody documentation",
      "Package and preserve samples appropriately",
      "Transport samples securely and track submission",
    ],
    when: [
      "When commodity quality is in dispute",
      "For pre-shipment or post-discharge testing",
      "When evidence must be independently handled",
      "For routine quality monitoring programmes",
    ],
    who: "Cargo owners, importers, exporters, insurers, manufacturers, legal teams, regulators.",
    deliv: ["Laboratory recommendation", "Sample collection record", "Chain-of-custody form", "Sample transport record", "Laboratory submission receipt"],
  },
];

// ── Pillar groupings ──────────────────────────────────────────────
const QA_TITLES    = ["Quality Assurance & Compliance", "Accreditation Support", "Certification Support", "Auditing", "Quality Management Portal", "Training and Capacity Building"];
const MARINE_TITLES = ["Marine Inspection & Surveying", "Condition Surveys", "Vessel Condition Surveys", "Damage & Loss Assessment", "Cargo Inspection & Surveying", "Sample Logistics", "Technical Reporting & Documentation"];

const qaServices     = services
  .filter(s => QA_TITLES.includes(s.title))
  .sort((a, b) => QA_TITLES.indexOf(a.title) - QA_TITLES.indexOf(b.title));
const marineServices = services
  .filter(s => MARINE_TITLES.includes(s.title))
  .sort((a, b) => MARINE_TITLES.indexOf(a.title) - MARINE_TITLES.indexOf(b.title));
const qmsService     = services.find(s => s.title === "Quality Management Portal")!;
        ...
        <div className="px-5 pb-6 border-t border-border bg-muted/20">
          ...
          <div className="mt-5">
            ...
            <ol className="grid sm:grid-cols-2 gap-2">
              {s.how.map((step, idx) => (
                <li key={idx} className="flex h-full gap-2.5 rounded-lg border border-border bg-muted/30 p-2.5 text-sm">
...
const PILLARS = [
  {
    ...
    chips: ["Certification & Accreditation", "Auditing & Assurance", "Training & Capacity Building"],
  },
  {
    id: "marine" as const,
    ...
    accentHex: "#0A6FAE",
    oneliner: "Independent inspection, surveying, sample logistics and technical reporting across vessels, cargo and marine assets.",
    chips: ["Marine Inspection", "Cargo Inspection", "Sample Logistics", "Technical Reporting"],
  },
] as const;
        ...
        {PILLARS.map((p) => {
          ...
          return (
            ...
            >
              ...
              <span className="sm:hidden">{p.id === "qa" ? "Quality Assurance" : "Inspection & Surveying"}</span>
            </button>
          );
        })}

        <div className="ml-auto text-xs text-muted-foreground hidden md:block">
          {activeTab === "qa" ? qaServices.length : marineServices.length} services
        </div>
      </div>
    </div>
  );
        ...
        <div className="p-8 md:p-12 grid md:grid-cols-2 gap-8 items-center">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <Link
                to="/services/quality-system-implementation"
...
function Services() {
  ...
  return (
    <SiteLayout>
      <PageHero
        eyebrow="Services"
        title="Quality Assurance, Inspection & Surveying Services"
        subtitle="Independent quality assurance, inspection, surveying, sample logistics and technical reporting services delivered with full accountability across Kenya and the region."
        variant="inspector"
      />
